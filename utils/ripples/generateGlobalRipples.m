...
varargin = optlistassign(varargin{:});
...
for ind = matchingInds'
    ...
    res = cellfetch(tetinfo, 'riptet');
    if all(cellfun(@isempty, res.values))
    ...
    if string(task(d, e).type) == "run"
        ...
        if ~isempty(velfilter)
            keyboard
            load(sprintf('%spos%02d.mat',animalprefix, d)); % get animalprefix [time, positions, velocity]
...
%         end
    ...
    clear out
    clear swslist
    ndb.save(globalripple, animalprefix, 1)
end
