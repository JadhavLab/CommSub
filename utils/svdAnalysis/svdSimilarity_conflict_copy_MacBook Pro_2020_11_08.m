 function [SimAnal, SimAnalTableComponent, SimAnalTableOverall, Patterns] = svdSimilarity(Patterns, varargin)
...
ip.addParameter('replicateDims', [1,2]);
ip.parse(varargin{:});
Opt = ip.Results;

% Compute max cell indices : we will need to remap each of the pattern
%acomponents to the actual cell numbers they represent
...
for i = progress(1:numel(Patterns),'Title','Singular value decomposition')
    ...
    uNew(sourceInds,sourceInds) = U;
    vNew(targetInds,targetInds) = V;
    sigNew(sourceInds,targetInds) = Sig;
...
indRepl = num2cell(indices(:,Opt.replicateDims),1);
    ...
    for field = progress(["U","V","UV"],'Title','fields')
        for ri = 1:R
            for rj = ri+1:R
                ...
                % Create SimAnalTableComponentOverall
