...
prefix = 0;   % [hostname] prefix for figure name
figfunc = @sfigure; % default figure function to use, 
grammObj = []; % gramm object?
...
if (~ischar(id))			% handle passed in
  ...
  name = sprintf('%s %d', id, ii);
	h = findobj('type', 'figure', 'name', name);
	if (isempty(h))
	  h = figfunc('name', sprintf('%s: %s %d', hostname, id, ii));
	  break;
	end
      end
    end
