function Out = match_cca(Patterns, Option, Spk, varargin)
...
for i = progress(1:numel(Patterns), 'Title', 'Time varying analysis - cca')
    ...
    O.source = tmp{1};
    O.target = tmp{2};
    O.match = analysis.temporal.single_match_cca(P, ...
                Option, Spk);
    O.method = "cca";
    O.name = P.name;
