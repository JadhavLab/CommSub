...
for a = 1:size(Behaviors,1)
    ...
    for phase = 1:4
        ...
        for p = 1:size(Behaviors,2)
            combined_hpc{a,phase,p} = repmat(zeros(height(beh{a,phase}), 7),30,1);
            combined_pfc{a,phase,p} = repmat(zeros(height(beh{a,phase}), 7),30,1);
        end
    end
end
%%
        ...
        for p = 1:size(Behaviors,2)
            ...
            for u = [1:numel(uGroups)]
                ...
                if usegramm
                    ...
                    D = C_hpc'; D = D(:); 
                    combined_hpc{a,phase,p} = [repmat([a, u, phase], N ,1), repelem(pat(1:KK)',TT), repelem(com(1:KK)',TT), repmat(tperf, KK, 1), D];
                    D = C_pfc'; D = D(:);
                    combined_pfc{a,phase,p} = [repmat([a, u, phase], N ,1), repelem(pat(1:KK)',TT), repelem(com(1:KK)',TT), repmat(tperf, KK, 1), D];
                    if u == numel(uGroups)
...
if usegramm
    ...
    combined_hpc = table(combined_hpc{:}, 'VariableNames', {'animal','epoch','phase','pattern','component','val'});
    combined_pfc = table(combined_pfc{:}, 'VariableNames', {'animal','epoch','phase','pattern','component','val'});
end
