...
def plot_hist(df_clean, thing = 'on_commsub_mag'):
    fig, axs = plt.subplots(len(uv_components), len(patterns), 
                            figsize=(4*len(patterns), 
                                     4*len(uv_components)), sharey=True)
    ...
    for i, uv_component in tqdm(enumerate(uv_components), total=len(uv_components)):
        # For each pattern
